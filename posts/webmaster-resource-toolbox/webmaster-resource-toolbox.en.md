---
title: Webmaster Resource Toolbox
published: 2026-06-23
created: 2026-06-23
updated: 2026-07-07
lastEdited: 2026-07-07
updateCount: 5
description: External resource entry points for domains, hosting, comments, monitoring, self-hosting, Cloudflare, analytics, webmaster communities, and blog frameworks.
image: ""
tags:
  - Webmaster Tools
  - Independent Blog
  - Self-hosting
category: Tools & Resources
draft: false
lang: en
translationKey: posts/webmaster-resource-toolbox/webmaster-resource-toolbox
aiSummary:
  generatedAt: "2026-07-11"
  model: "codex-local"
  items:
    - "External list of resources for domains, hosting, comments, monitoring, and self-hosting."
    - "Also covers Cloudflare, analytics, and personal site communities."
    - "Always double-check pricing, terms, and availability for each service yourself."
alias: blog-resource-toolbox

---

This post only lists external resource links.

To learn how to read articles on this site, see [[site-article-index|Site Article Index]].

For commonly used local software, see [[useful-free-software-toolbox|Useful Free Software List]].

Rules and free tiers can change, so always check the official website before signing up.

## GitHub List Repositories
- [Free for Developers](https://free-for.dev/#/): Collection of free services for developers, 100k+ GitHub stars.
- [awesome-cloudflare](https://github.com/zhuima/awesome-cloudflare): Collection of Cloudflare uses, 10k+ GitHub stars.
- [sindresorhus/awesome: 😎 Awesome lists about all kinds of interesting topics](https://github.com/sindresorhus/awesome#readme): Awesome lists post, 400k+ GitHub stars.

## Free Domains and Temporary Entry Points

- [DNSHE](https://my.dnshe.com/): Free subdomain, good for practicing DNS first.
- [eu.org](https://nic.eu.org/): Classic free domain, but approval is often slow.
- [us.kg](https://nic.us.kg/): Also quite common, check current rules before registering.
- [pp.ua](https://nic.ua/): One of the established free domain entry points.
- [GitHub Pages](https://pages.github.com/): Will give you a `github.io`.
- [Cloudflare Pages](https://pages.cloudflare.com/): Will give you a `pages.dev`.

If you're just starting a website, these should keep you busy for a while.

## Official Domain Registration

- [Namesilo](https://www.namesilo.com/)
- [Porkbun](https://porkbun.com/)
- [Cloudflare Registrar](https://www.cloudflare.com/products/registrar/)

I personally prioritize checking the first-year price and renewal price, otherwise it's easy to get lured in by a low first-year cost.

## Static Hosting

- [Cloudflare Pages](https://pages.cloudflare.com/): I'm currently using it myself.
- [Vercel](https://vercel.com/): Many front-end projects are hosted here.
- [Netlify](https://www.netlify.com/)
- [GitHub Pages](https://pages.github.com/)
- [Render](https://render.com/)

For a purely static blog, Cloudflare Pages and GitHub Pages are generally sufficient.

## Comment Systems

- [Giscus](https://giscus.app/)
- [Twikoo](https://twikoo.js.org/)
- [Artalk](https://artalk.js.org/)
- [Waline](https://waline.js.org/)
- [Cusdis](https://cusdis.com/)

If you write a Chinese blog, I think Twikoo and Waline are more common.

## Monitoring and Status Pages

- [Gatus](https://gatus.io/)
- [Uptime Kuma](https://github.com/louislam/uptime-kuma)
- [UptimeRobot](https://uptimerobot.com/)
- [Better Stack](https://betterstack.com/)

For self-hosting, I'd look at Uptime Kuma first; it saves a lot of hassle.

## Notifications and Push Services

- [ntfy](https://ntfy.sh/)
- [Bark](https://github.com/Finb/Bark)
- [Gotify](https://gotify.net/)
- [PushDeer](https://github.com/easychen/pushdeer)
- [Server 酱](https://sct.ftqq.com/)

It's really quite necessary to send yourself a notification after a script runs.

## Self-hosting Resource Index

- [awesome-selfhosted](https://awesome-selfhosted.net/)
- [awesome-selfhosted GitHub](https://github.com/awesome-selfhosted/awesome-selfhosted)
- 
- [selfh.st](https://selfh.st/)
- [noted.lol](https://noted.lol/)

These are good to browse when you're not sure what to install.

## Cloudflare Related

- [Cloudflare Docs](https://developers.cloudflare.com/)
- [Pages Docs](https://developers.cloudflare.com/pages/)
- [Workers Docs](https://developers.cloudflare.com/workers/)
- [R2 Docs](https://developers.cloudflare.com/r2/)
- [Tunnel Docs](https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/)

For something like Cloudflare, it's best to check the official documentation directly, otherwise community posts can quickly become outdated.

## Storage and CDN

- [Cloudflare R2](https://developers.cloudflare.com/r2/)
- [Backblaze B2](https://www.backblaze.com/cloud-storage)
- [GitHub Releases](https://github.com/)
- [jsDelivr](https://www.jsdelivr.com/)

These are often used for image hosting, small files, and backup scenarios.

## Analytics

- Cloudflare Web Analytics: Enable directly in Cloudflare dashboard.
- [Umami](https://umami.is/)
- [Plausible](https://plausible.io/)
- [GoatCounter](https://www.goatcounter.com/)

I personally prefer Umami these days; the interface looks clean.

## Webmaster Communities and Directories

- [开往 Travellings](https://www.travellings.cn/)
- [萌备](https://icp.gov.moe/)
- [十年之约](https://www.foreverblog.cn/)
- [揪蝉](https://hi.jiuchan.org/docs)
- [中文独立博客列表](https://github.com/timqian/chinese-independent-blogs)
- [BlogFinder](https://bf.zzxworld.com/)
- [博友圈](https://www.boyouquan.com/)
- [积薪](https://firewood.news/)

These are more like entry points you'd use to get your site out there after you've set it up.

## Forums and Tech Communities

- [Linux DO](https://linux.do/)
- [V2EX](https://www.v2ex.com/)
- [NodeSeek](https://www.nodeseek.com/)
- [NodeLoc](https://www.nodeloc.com/)
- [Hacker News](https://news.ycombinator.com/)

If you want to see website building, VPS, freebies, and tinkering experiences, I often browse these myself.

## Blog Frameworks

- [Astro](https://astro.build/)
- [Hugo](https://gohugo.io/)
- [Hexo](https://hexo.io/)
- [VitePress](https://vitepress.dev/)
- [Zola](https://www.getzola.org/)

I'm mainly using Astro right now, so I'm more familiar with it in this area.

## Passwords and Security

- [Vaultwarden](https://github.com/dani-garcia/vaultwarden)
- [Bitwarden](https://bitwarden.com/)
- Cloudflare Access: Enable in Cloudflare dashboard.
- [Let's Encrypt](https://letsencrypt.org/)
- [Caddy](https://caddyserver.com/)

Don't expose your admin panel directly to the public internet, and don't actually rely on memory for passwords.
